...
You can also use other relevance score providers like Voyage AI by MongoDB, Cohere, or ZeroEntropy:
...
Voyage AI by MongoDB provides dedicated reranking models. `rerank-2.5` and `rerank-2.5-lite` both allow up to 32,000 tokens for the query and any single document combined, and up to 600,000 tokens across a request. The newer `rerank-3` and `rerank-3-lite` models (in preview) also support a 32,000-token context. `VoyageRelevanceScorer` reads `VOYAGE_API_KEY` from the environment, or accepts an `apiKey` in its config.
